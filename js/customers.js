...
function getFiltered() {
  ...
  if (state.sort) {
    ...
    sorted.sort((a,b) => {
      ...
      const r = Number.isFinite(n) && av !== "" && bv !== "" ? n : String(av).localeCompare(String(bv), undefined, {numeric:true,sensitivity:"base"});
      return dir === "desc" ? -r : r;
    });
  }
  return sorted;
...
function render() {
  ...
  document.getElementById("clearCustomerSearch").onclick = () => { state.q=""; state.page=1; input.value=""; updateResults(); input.focus(); };
...
function updateResults() {
  ...
  document.getElementById("customerRows").innerHTML = p.items.map(row).join("") || `<tr><td colspan="6"><div class="empty">No customers found.</div></td></tr>`;
  document.getElementById("customerCount").textContent = `${all.length} result${all.length === 1 ? "" : "s"}`;
  document.getElementById("customerPagination").innerHTML = paginationHtml(p.page,p.total);
...
function row(x) { return `<tr><td><div class="person"><div class="table-avatar">${escapeHtml(initials(x.name))}</div><div><strong>${escapeHtml(x.name)}</strong><small>${escapeHtml(x.email)}</small></div></div></td><td>${escapeHtml(x.city)}</td><td>${escapeHtml(x.company)}</td><td>${escapeHtml(x.phone)}</td><td><span class="badge ${x.status === "Active" ? "badge-success" : "badge-neutral"}">${escapeHtml(x.status)}</span></td><td><div class="actions"><button class="btn btn-light" data-edit="${x.id}">Edit</button><button class="btn btn-danger" data-delete="${x.id}">Delete</button></div></td></tr>`; }
function initials(name) { return String(name||"").split(/\s+/).filter(Boolean).map(x=>x[0]).slice(0,2).join("").toUpperCase(); }

function form(id) {
  const x = getCollection("customers").find(a=>Number(a.id)===Number(id)) || {name:"",email:"",phone:"",company:"",city:"",status:"Active"};
  openModal({title:id?"Edit Customer":"Add Customer",body:`<form id="customerForm" class="form-grid"><div class="field"><label>Name</label><input name="name" value="${escapeHtml(x.name)}" required></div><div class="field"><label>Email</label><input name="email" type="email" value="${escapeHtml(x.email)}" required></div><div class="field"><label>Phone</label><input name="phone" value="${escapeHtml(x.phone)}"></div><div class="field"><label>City</label><input name="city" value="${escapeHtml(x.city)}"></div><div class="field"><label>Company</label><input name="company" value="${escapeHtml(x.company)}"></div><div class="field"><label>Status</label><select name="status"><option ${x.status === "Active" ? "selected" : ""}>Active</option><option ${x.status === "Inactive" ? "selected" : ""}>Inactive</option></select></div></form>`,footer:`<button class="btn btn-light" data-close>Cancel</button><button class="btn btn-primary" id="saveCustomer">Save Customer</button>`});
  document.getElementById("saveCustomer").onclick=()=>{const formEl=document.getElementById("customerForm");if(!formEl.reportValidity())return;const data=Object.fromEntries(new FormData(formEl));let list=getCollection("customers");list=id?list.map(v=>Number(v.id)===Number(id)?{...data,id:Number(id)}:v):[...list,{...data,id:nextId(list)}];saveCollection("customers",list);closeModal();toast("Customer saved.");updateResults();};
}
function remove(id){if(!confirm("Delete this customer?"))return;saveCollection("customers",getCollection("customers").filter(x=>Number(x.id)!==Number(id)));toast("Customer deleted.","warning");state.page=1;updateResults();}
