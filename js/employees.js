...
function getFiltered() {
  ...
  const fields = ["id", "name", "email", "phone", "role", "department", "salary", "status"];
  const filtered = query
    ? source.filter(item => fields.some(key => normalize(item[key]).includes(query)))
    : source;
  ...
  if (state.sort) {
    ...
    sorted.sort((a, b) => {
      ...
      const result = Number.isFinite(numeric) && av !== "" && bv !== ""
        ? numeric
...
function render() {
  ...
  document.getElementById("clearEmployeeSearch").onclick = () => {
    ...
    input.value = ""; updateResults(); input.focus();
  };
...
function updateResults() {
  ...
  document.getElementById("employeeRows").innerHTML = result.items.map(row).join("") || `<tr><td colspan="6"><div class="empty">No employees found.</div></td></tr>`;
  document.getElementById("employeeCount").textContent = `${getFiltered().length} result${getFiltered().length === 1 ? "" : "s"}`;
  document.getElementById("employeePagination").innerHTML = paginationHtml(result.page, result.total);
  document.querySelectorAll("[data-page]").forEach(btn => btn.onclick = () => { state.page = Number(btn.dataset.page); updateResults(); });
  document.querySelectorAll("[data-edit]").forEach(btn => btn.onclick = () => form(Number(btn.dataset.edit)));
  document.querySelectorAll("[data-delete]").forEach(btn => btn.onclick = () => remove(Number(btn.dataset.delete)));
}
...
function form(id) {
  const x = getCollection("employees").find(a => Number(a.id) === Number(id)) || {name:"",email:"",phone:"",role:"Employee",department:"Sales",salary:0,status:"Active"};
  openModal({title:id ? "Edit Employee" : "Add Employee", body:`<form id="employeeForm" class="form-grid"><div class="field"><label>Name</label><input name="name" value="${escapeHtml(x.name)}" required></div><div class="field"><label>Email</label><input name="email" type="email" value="${escapeHtml(x.email)}" required></div><div class="field"><label>Phone</label><input name="phone" value="${escapeHtml(x.phone)}"></div><div class="field"><label>Department</label><input name="department" value="${escapeHtml(x.department)}"></div><div class="field"><label>Role</label><input name="role" value="${escapeHtml(x.role)}"></div><div class="field"><label>Salary</label><input name="salary" type="number" min="0" value="${Number(x.salary)||0}"></div><div class="field"><label>Status</label><select name="status"><option ${x.status === "Active" ? "selected" : ""}>Active</option><option ${x.status === "Inactive" ? "selected" : ""}>Inactive</option></select></div></form>`, footer:`<button class="btn btn-light" data-close>Cancel</button><button class="btn btn-primary" id="saveEmployee">Save Employee</button>`});
  document.getElementById("saveEmployee").onclick = () => {
    ...
    list = id ? list.map(v => Number(v.id) === Number(id) ? {...data, id:Number(id)} : v) : [...list, {...data, id:nextId(list)}];
    saveCollection("employees", list); closeModal(); toast("Employee saved."); updateResults();
  };
}

function remove(id) {
  if (!confirm("Delete this employee?")) return;
  saveCollection("employees", getCollection("employees").filter(x => Number(x.id) !== Number(id)));
  toast("Employee deleted.", "warning"); state.page = 1; updateResults();
}
